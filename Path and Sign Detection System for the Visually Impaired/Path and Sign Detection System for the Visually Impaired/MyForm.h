	...
	private:
		...
		void PlayAudioAlert(System::String^ soundType) {
			...
			try {
				...
				// ===== [แก้ไขจุดที่ 1] เปลี่ยนชื่อประเภทและชื่อไฟล์เสียงเป็น zebra_crossing.wav =====
...
#pragma endregion
		...
		// =========================================================
		// Event สำหรับปุ่ม BROWSE และ START/STOP
		// =========================================================
	...
	private: System::Void bgWorker_DoWork(System::Object^ sender, System::ComponentModel::DoWorkEventArgs^ e) {
		...
		while (true) {
			...
			if (isZebraFound) {
				...
				// ===== [แก้ไขจุดที่ 2] สั่งเล่นเสียงโดยใช้ Key ใหม่ที่ตั้งไว้ =====
